...
from config import (WINDOW_BG_COLOR, HEADER_BG_COLOR, FOOTER_BG_COLOR, PRIMARY_COLOR,
                   ACCENT_COLOR, TEXT_COLOR, SECONDARY_TEXT_COLOR, DISABLED_TEXT_COLOR,
                   APP_NAME, VERSION, COMPANY_NAME, APP_AUTHOR, LOGO_CANDIDATES)
...
class HeaderFrame(tk.Frame):
    ...
    def _load_logo(self):
        ...
        for candidate in LOGO_CANDIDATES:
            logo_path = Path(__file__).parent / candidate
            if logo_path.exists():
                try:
                    self.logo_image = tk.PhotoImage(file=str(logo_path))
                    app_logger.info(f"Logo loaded: {logo_path}")
                    break
                except Exception as e:
                    app_logger.warning(f"Failed to load {logo_path}: {e}")
    ...
    def _create_widgets(self):
        ...
        # Title frame
        self.title_frame = tk.Frame(self, bg=HEADER_BG_COLOR)
...
class FooterFrame(tk.Frame):
    ...
    def __init__(self, parent, **kwargs):
        super().__init__(parent, bg=FOOTER_BG_COLOR, padx=10, pady=6, **kwargs)
        self._create_widgets()
